    ...
    new_cards = '''<!-- Tarjetas de KPIs -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        ...
        <div class="metric-card bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col justify-center transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <p class="text-3xl font-black text-slate-800 tracking-tight">{{ resumen()?.ventasHoy | currency:'USD':'symbol':'1.0-0' }}</p>
        ...
        <div class="metric-card bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col justify-center transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <p class="text-3xl font-black text-amber-500 tracking-tight">{{ resumen()?.ventasSemana | currency:'USD':'symbol':'1.0-0' }}</p>
        ...
        <div class="metric-card bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col justify-center transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <p class="text-3xl font-black text-emerald-600 tracking-tight">{{ resumen()?.ventasMes | currency:'USD':'symbol':'1.0-0' }}</p>
        ...
        <div class="metric-card bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col justify-center transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <p class="text-3xl font-black text-indigo-500 tracking-tight">{{ resumen()?.devolucionesHoy | currency:'USD':'symbol':'1.0-0' }}</p>
          <p class="text-xs text-gray-400 mt-1">{{ resumen()?.ticketsDevolucionesHoy }} devoluci\u00f3n(es)</p>
        </div>

        <!-- KPI Sin Stock -->
        <div class="metric-card bg-red-50/50 rounded-lg shadow-sm border border-red-200 p-4 flex flex-col justify-center transition-all hover:-translate-y-1 hover:shadow-md">
          <p class="text-[11px] font-bold text-red-400 uppercase tracking-wider mb-1">Sin Stock</p>
          <p class="text-3xl font-black text-red-600 tracking-tight">{{ resumen()?.sinStock }}</p>
