...
// Delete the old divs
html = html.replace(/<div class="space-y-3 border-b border-slate-700 pb-4 mb-4">[\s\S]*?<div class="text-right">\s*<span class="text-xs text-slate-400 block uppercase">MXN<\/span>\s*<span class="text-3xl font-bold text-yellow-400">{{ totalCompra\(\) \| currency:'MXN' }}<\/span>\s*<\/div>\s*<\/div>/, newStr);

fs.writeFileSync('src/app/features/compras/nueva-compra.component.html', html, 'utf8');
console.log("HTML replaced with regex");
